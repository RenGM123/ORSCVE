...
def aignloss(cur_y, next_y, gt_y, n_flow):
    """
    只用 ANCHOR2_LQ (next_y) 和对应光流 n_flow，对齐到当前帧/GT。
    这相当于 multi_scale_feat_align 只对 ANCHOR2 分支做约束。
    """
    next_warp = warp(next_y, n_flow)
    n_align = F.l1_loss(next_warp, gt_y)
    return n_align
