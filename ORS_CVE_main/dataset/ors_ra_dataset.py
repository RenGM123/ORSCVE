...
class _BaseORSRADataset(Dataset):
    """
    Base dataset for ORS-RA.

    ORS for RA coding:
        - if t is TL0 frame (t % gop == 0), use next TL0: t + gop
        - else, use next-next TL0: (t // gop + 2) * gop

    This is designed to satisfy ORS in RA:
        1) outside PredPath(t) as much as possible
        2) prefer lowest temporal layer (TL0)
        3) temporally as close as possible under 1)+2)

    Tail handling:
        - If ideal anchor exceeds sequence length, fallback to the nearest future TL0.
        - If no future TL0 exists but there is still a future frame, fallback to the last frame.
        - Frames with no future frame at all are skipped during dataset construction.
    """
    ...
    def __init__(
        ...
        for video_idx, video_name in enumerate(video_names):
            ...
            # RA special:
            # The last frame has no future frame, so it cannot use future ORS.
            # Therefore skip frame_idx == num_frames - 1.
            valid_num = max(0, num_frames - 1)

            for frame_idx in range(valid_num):
    ...
    def _select_ors_reference_index(self, frame_idx: int, num_frames: int) -> int:
        """
        ORS reference selection for RA coding.
        ...
        Tail handling:
            - If ideal anchor is within sequence length, use it.
            - Else fallback to nearest future TL0.
            - If no future TL0 exists but there is future frame, use last frame.
            - If no future frame exists, raise error.
              But normally this case is skipped during dataset construction.
        """
        t = int(frame_idx)
        ...
        if t >= num_frames - 1:
            raise ValueError(
                f"frame_idx={t} has no future frame in sequence with "
                f"num_frames={num_frames}."
            )

        # 1. Ideal RA ORS anchor.
        if t % self.gop == 0:
            ideal_idx = t + self.gop
        else:
            ideal_idx = (t // self.gop + 2) * self.gop

        if ideal_idx < num_frames:
            return ideal_idx

        # 2. Fallback to nearest future TL0.
        # Future TL0 means k * gop > t and k * gop < num_frames.
        next_tl0 = ((t // self.gop) + 1) * self.gop

        if next_tl0 > t and next_tl0 < num_frames:
            return next_tl0

        # 3. If no future TL0 exists but future frame exists, use last frame.
        return num_frames - 1
...
class TrainORSRADataset(_BaseORSRADataset):
    """
    Training split for ORS-RA.
...
class ValORSRADataset(_BaseORSRADataset):
    """
    Validation split for ORS-RA.
